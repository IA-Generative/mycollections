...
l'API : `python -m app.services.amorces.cli natinf [--max-documents N]`.
...
async def _jouer(ident: str, max_documents: int | None) -> int:
...
def main() -> None:
    ...
    args = p.parse_args()
    sys.exit(asyncio.run(_jouer(args.amorce, args.max_documents)))
